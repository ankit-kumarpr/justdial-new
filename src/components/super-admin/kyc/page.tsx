...
const KycTable = ({ submissions, onStatusUpdate }: { submissions: KycSubmission[], onStatusUpdate: () => void }) => {
    ...
    const handleViewDetails = (submission: KycSubmission) => {
        const submissionWithFullUrls = {
            ...submission,
            aadharImage: submission.aadharImage && !submission.aadharImage.startsWith('http') 
                ? `${apiBaseUrl}${submission.aadharImage}` 
                : submission.aadharImage,
            videoKyc: submission.videoKyc && !submission.videoKyc.startsWith('http') 
                ? `${apiBaseUrl}${submission.videoKyc}` 
                : submission.videoKyc,
        };
        setSelectedKyc(submissionWithFullUrls);
